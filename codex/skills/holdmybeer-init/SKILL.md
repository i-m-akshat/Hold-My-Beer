...
## Objective
Detect existing SDD structures (.holdmybeer, .holdmybeer, .gsd, etc.) and initialize a premium HoldMyBeer SDD workspace if none are found.
...
- A newly initialized `.holdmybeer/` folder with templates (`specify.md`, `plan.md`, `tasks.md`, `constitution.md`, and a premium interactive `dashboard.html`).
...
3. If none exist:
   - Confirm and initialize a `.holdmybeer/` directory.
   - Create these files:
     - `.holdmybeer/specify.md`: Spec template (the 23 sections: Executive Summary, Goals, Non-goals, Scope, Functional, Non-functional, Business Rules, Data Model, API, UI, Permissions, Security, Concurrency, Observability, Edge Cases, Error Handling, Validation, Performance, Acceptance Criteria, Out of Scope, Open Questions, Risks, Needs Clarification).
     - `.holdmybeer/plan.md`: Roadmap phases (Phases 1, 2, ... with objective, files affected, dependencies, risks, testing) + highlights.
     - `.holdmybeer/tasks.md`: Task checklist.
     - `.holdmybeer/constitution.md`: Coding standards, layered architecture rules, security, concurrency, testing, and observability guidelines.
     - `.holdmybeer/dashboard.html`: A premium local HTML dashboard (dark mode, glassmorphic card layouts,Outfit font, gold/purple gradients, tab switching for files, copy-to-clipboard command shortcuts, and an interactive SDD pipeline progress visualizer).
